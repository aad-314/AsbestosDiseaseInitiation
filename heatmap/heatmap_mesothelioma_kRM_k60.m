% Heatmap for CM and SC for various inputs of kRM and k60
% kRM=j, and k60=c where j and c are varied in code below
...
for j = 0.4:0.1:1.4
    ...
    for c = 49.5:.1:50.5
        ...
        kRM= j; %recruitment rate from monocytes VALID
        ...
        k60=c; % now Tcell carrying capacity, adjust for immune response of T cells. If set to 200, default patient will not develop cancer.
...
%----------------------------------------------
figure(1) %1
[X,Y] = meshgrid(0.4:0.1:1.4,49.5:.1:50.5);%j range, c range
...
figure(2) %2
[X,Y] = meshgrid(0.4:0.1:1.4 ,49.5:.1:50.5);%j range, c range
h = surf(X,Y,TimeMatrix2);
view(0,90)
colormap(hot)
colorbar
h1 = xlabel('kRM=j');
h2 = ylabel('k60=c');
h3 = title('SC at time 9000');
set( findobj(gca,'type','line'), 'LineWidth', 5);
set(gca, 'FontSize', 10,'LineWidth',4)
box on;
